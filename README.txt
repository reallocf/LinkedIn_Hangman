...
Extensions:
    ...
    These include:
        ...
        Adding support for coloring and improving layout within the command line interface
    ...
    STEP ONE, -best goes through all of the words in the dictionary of the correct size (found byanother api request without knowledge of the difficulty level) and tries to figure out which words it can eliminate.

    It can eliminate a word for one of two reasons:
    ...
    STEP TWO, -best now loops through every word and decides to remove the letter that occurs in the most words. This does one of two things:
        ...
        If it guesses correctly, that's great! You're know one letter closer to solving the problem.

        If -best guesses incorrectly, that's even better! Now you've minimized the maximum amount of words for the next time -best is called.

    And it does all of this in O(n) time relative to the total number of characters returned from the api call.

    This takes a bit of time due to the immense size of the provided dictionary, but is still accomplished in less than a few seconds even if used as the very first guess.


    Ultimately, -best is designed to be a game playing agent that can help you solve your hangman challenge even when it's really difficult.
    ...
    -v is used to better visualize what -best is doing at every step. When on, -v displays details about each of the two steps. Namely:
        ...
        1. A list of all the words after step one filters through them

        2. A tally of the counts for each letter with the selected letter highlighted in green as generated by step two
    ...
    In terms of extending -best, I'd be interested in learning more about how prefix trees (tries) work to see if those would be useful. I know they're used for autocompletion and challenges like that, so maybe there is an interesting overlap.
